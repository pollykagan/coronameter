...
<?php $begin('main_body_field') ?>
                <p class="lead offset-2" style="text-align: left"></p>
                <main>
                    <section>
...
<?php $end('main_body_field') ?>
 ...
 <?php $begin('main_body_end') ?>
                <form method="post">
                    <button onclick="window.location='index.html'" type="button" class="btn btn-primary pt-2">Go home</button>
                </form>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    */
    ...
    groupsArcs
        ...
        .html(({ data: d }) => `
    <tspan x="0">${d.name}:</tspan> ${d.value}%</tspan>
  `)
        .style('opacity', 0)
